...
const orderController = {
    ...
    postNewOrder: async (req,res,next)=>{
        try {
            ...
            const createCustomer = await OrderModel.create({customerId,productId,quantity})
            res.status(201).send({ data:{newOrder: createCustomer,quantity_left:newQuantity }, message: 'Register successful!', success: true });
        }
        catch(error){
            next(error)
        }
